...
# TODO – Developer: Update the regex patterns ("YOUR-REGEX-HERE") to match files that should trigger your
# ingest pipeline.
...
mapping: Dict["AnyStr@compile", IngestSpec] = {
    ...
    re.compile(r"\d{4}_FLT.CSV"): IngestSpec(
        pipeline=Pipeline,
        pipeline_config=expand("config/pipeline_config_wave.yml", __file__),
        storage_config=expand("config/storage_config_wave.yml", __file__),
        name="wave",
    ),
