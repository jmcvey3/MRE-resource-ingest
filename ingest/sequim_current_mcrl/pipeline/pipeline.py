    ...
    def hook_generate_and_persist_plots(self, dataset: xr.Dataset):
        ...
        with self.storage._tmp.get_temp_filepath(filename) as tmp_path:
            ...
            magn = ax[0].pcolormesh(
                date, ds.range, ds.U_mag, cmap="Blues", shading="nearest"
            )
            ax[0].plot(date, ds.depth)
            ax[0].set_xlabel("Time (UTC)")
            ax[0].set_ylabel(r"Range [m]")
            ax[0].set_ylim([0, 11])
            add_colorbar(ax[0], magn, r"Current velocity [m/s]")

            dirc = ax[1].pcolormesh(
                date, ds.range, ds.U_dir, cmap="twilight", shading="nearest"
            )
            ax[1].plot(date, ds.depth)
            ax[1].set_xlabel("Time (UTC)")
            ax[1].set_ylabel(r"Range [m]")
            ax[1].set_ylim([0, 11])
            add_colorbar(ax[1], dirc, r"Current dir [deg from N]")
